<div x-show="isEditModalOpen" x-cloak style="display: none;"
    ...
    <div @click.away="isEditModalOpen = false"
        ...
        <div class="flex items-center justify-between mb-5">
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white/90">Edit Donatur: <span
                    x-text="editForm.restaurant_name" class="text-brand-500"></span></h3>
        ...
        <form :action="'/admin/donors/' + editForm.id" method="POST" class="space-y-4">
            @csrf
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Restoran /
                    Usaha</label>
                <input type="text" name="restaurant_name" x-model="editForm.restaurant_name" required
                    class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:text-white">
            </div>
            ...
            <div class="mt-6 flex justify-end gap-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                ...
                <button type="submit"
                    class="rounded-lg bg-warning-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-warning-600 transition shadow-theme-xs">Update
                    Data</button>
            </div>
        </form>
    </div>
