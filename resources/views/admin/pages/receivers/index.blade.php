...
@section('content')
    @php
        $mappedReceivers = $receivers->map(function ($r) {
            return [
                'id' => (string) $r->_id,
                'name' => $r->name,
                'email' => $r->email,
    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-title-md2 font-bold text-black dark:text-white">Data Penerima (Yayasan)</h2>
            <nav>
                <ol class="flex flex-wrap items-center gap-1.5">
                    ...
                    <li class="flex items-center gap-1.5"><span class="text-gray-500 dark:text-gray-400">/</span><span
                            class="text-sm font-medium text-gray-800 dark:text-white/90">Receivers</span></li>
                </ol>
            </nav>
        </div>
        ...
        @if (session('success'))
            <div x-data="{ showAlert: true }" x-show="showAlert" x-transition
                ...
                <div class="w-full">
                    <h5 class="text-lg font-semibold text-gray-800 dark:text-white/90">Success!</h5>
        ...
        @if ($errors->any())
            <div x-data="{ showAlert: true }" x-show="showAlert" x-transition
                class="mb-5 flex w-full items-start border-l-6 border-error-500 bg-error-50 px-7 py-4 shadow-theme-md dark:border-error-500 dark:bg-gray-dark rounded-r-lg">
                <div class="mr-5 flex h-9 w-full max-w-[36px] items-center justify-center rounded-lg bg-error-500"><svg
                ...
                <button @click="showAlert = false" type="button"
                    class="ml-auto p-1.5 rounded-lg text-gray-500 hover:bg-error-200 transition"><svg class="w-5 h-5"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12">
                        </path>
                    </svg></button>
        ...
        <div x-data="receiverTable()">

            @include('admin.pages.receivers.components.create-modal')
            @include('admin.pages.receivers.components.edit-modal')
            @include('admin.pages.receivers.components.delete-modal')

            <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex flex-col gap-4 px-5 mb-4 md:flex-row md:items-center md:justify-between sm:px-6">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Daftar Yayasan / Penerima</h3>
                    </div>
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                        <div class="relative">
                            ...
                            <input type="text" x-model="search" placeholder="Cari yayasan/pengurus..."
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr class="border-gray-200 border-y dark:border-gray-700">
                                    ...
                                    <th
                                        ...
                                        Nama Yayasan</th>
                                    <th
                                        class="px-4 py-3 font-medium text-gray-500 text-start text-theme-sm dark:text-gray-400 min-w-[150px]">
                                        Titik Lokasi</th>
                                    <th
                                        class="px-4 py-3 font-medium text-gray-500 text-start text-theme-sm dark:text-gray-400">
                                        Kapasitas</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                <tr x-show="paginatedItems.length === 0" x-cloak>
                                    <td colspan="5" class="py-6 text-center text-gray-500 dark:text-gray-400">Tidak ada
                                        data ditemukan.</td>
                                </tr>
                                            ...
                                            <div class="flex items-center">
                                                ...
                                                <div class="ml-4">
                                                    ...
                                                    <div class="text-sm text-gray-500 dark:text-gray-400"
                                                        x-text="r.phone + ' | ' + r.email"></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="text-sm font-semibold text-gray-800 dark:text-white/90"
                                                x-text="r.foundation_name"></div>
                                        </td>
                                        <td class="px-4 py-4">
                                            <div class="text-sm text-gray-500 dark:text-gray-400">
                                                <template x-if="r.latitude !== null && r.longitude !== null">
                                                    <span>
                                                        <span class="block text-xs font-mono text-brand-500">Lat: <span
                                                                x-text="r.latitude"></span></span>
                                                        <span class="block text-xs font-mono text-brand-500">Lng: <span
                                                                x-text="r.longitude"></span></span>
                                                    </span>
                                                </template>
                                                <template x-if="r.latitude === null || r.longitude === null">
                                                    <span class="italic text-gray-400 text-xs">Belum ada lokasi</span>
                                                </template>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                                            <div class="flex items-center justify-end space-x-3">
                                                <div class="flex space-x-2">
                                                    <template x-if="!r.is_verified">
                                                        <form :action="'/admin/receivers/' + r.id + '/verify'"
                                                    ...
                                                    <template x-if="r.is_verified">
                                                        <form :action="'/admin/receivers/' + r.id + '/reject'"
                                                            method="POST" class="inline">
                                                ...
                                                <button type="button" @click="openEditModal(r)" title="Edit Data"
                                                ...
                                                <button type="button" @click="openDeleteModal(r)" title="Hapus Data"
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('receiverTable', () => ({
                ...
                allItems: @json($mappedReceivers),
                itemsPerPage: 10,
                currentPage: 1,

                openEditModal(receiver) {
                    this.editForm = {
                        ...receiver
                    };
                    this.editForm.latitude = receiver.latitude || -6.200000;
                    this.editForm.longitude = receiver.longitude || 106.816666;
                ...
                openDeleteModal(receiver) {
                    this.deleteForm = {
                        id: receiver.id,
                        foundation_name: receiver.foundation_name
                    };
                ...
                init() {
                    ...
                    if (this.isModalOpen) setTimeout(() => {
                        initLeafletMap();
                        if (map) map.invalidateSize();
                    }, 200);
                }
